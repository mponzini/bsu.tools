#' @title Create a new Project Details Document from Template
...
new_project_details <- function(
  ...
  gist <- "gists/project_details.qmd"
  template_path <- system.file(gist, package = 'bsu.tools')

  if (template_path == "") {
    stop("Could not find project details template in package installation")
  }
